...
<html>
    <head>
        ...
        <title>Cập nhật nhà cung cấp dịch vụ </title>
    ...
    <body>
        <div class="container">
            ...
            <c:set var ="i" value="${requestScope.serviceprovider}"/>
            <form action="updateServiceProvider" method="post" id="serviceproviderForm" onsubmit="return validateForm()">
                <input type="hidden" name="id" value="${i.provider_id}"/>
                <div class="form-group">
                    <label for="serviceprovider_name">Họ và tên:</label>
                    <input type="text" id="serviceprovider_name" name="serviceprovider_name" required value ="${i.name}" />
                    <div class="invalid-feedback" id="serviceprovider_name-feedback">Hãy điền đúng tên hợp lệ.</div>
                </div>

                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="email" id="email" name="email" required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" value ="${i.email}"/>
                    <div class="invalid-feedback" id="email-feedback">Hãy điền địa chỉ email hợp lệ .</div>
                </div>

                <div class="form-group">
                    <label for="username">Tên đăng nhập:</label>
                    <input type="text" id="username" name="username" required value="${i.username}" />
                </div>

                <div class="form-group">
                    <label for="phone_number">Số điện thoại:</label>
                    <input type="tel" id="phone_number" name="phone_number" required 
                           pattern="[0-9]{10}" maxlength="10" value="${i.phone_number}" />
                    <div class="invalid-feedback" id="phone-feedback">Hãy điền số điện thoại chứa 10 số hợp lệ.</div>
                </div>

                <div class="form-group">
                    <label for="address">Địa chỉ:</label>
                    <input type="text" id="address" name="address" required value="${i.address}"/>
                </div>

                <div class="form-group">
                    <label for="servicetype">Loại dịch vụ:</label>
                    <select id="servicetype" name="servicetype" class="custom-select" required>
                        <option value="Electricity" ${i.servicetype =="Electricity" ? "selected" : ""}>Điện</option>
                        <option value="Water" ${i.servicetype =="Water" ? "selected" : ""}>Nước</option>
                        <option value="Internet"  ${i.servicetype =="Internet" ? "selected" : ""}>Internet</option>
                    </select>
                </div>
