...
<form action="buyInsurance" method="post" enctype="multipart/form-data">
    ...
    <div class="container">
        ...
        <select id="loanDropdown" class="filter-dropdown" name="loan_id" onchange="updateLoanDetails()">
            <c:if test="${not empty listLoan}">                        
                <c:forEach var="l" items="${requestScope.listLoan}">
                    <option value="${l.loan_id}" ${l.loan_id == param.loan_id ? 'selected' : ''}>
                        ${l.notes}
                    </option>  
                </c:forEach>                
            </c:if>
        ...
        <select id="policyDropdown" class="filter-dropdown" name="policy_id" onchange="updatePolicyDetails()">
            <c:if test="${not empty listPolicy}">
                <c:forEach var="p" items="${requestScope.listPolicy}">
                    <option value="${p.policy_id}" ${p.policy_id == param.policy_id ? 'selected' : ''}>
                        ${p.policy_name}
                    </option>  
                </c:forEach>                
            </c:if>
        ...
        <input type="text" name="contract_name" value="${param.contract_name}" required /> 

        <label>Số tiền được nhận</label>
        <input type="text" id="coverageAmount" name="coverage_amount" value="${param.coverage_amount}" required readonly /> 

        <label>Số tiền cần đóng</label>
        <input type="text" id="premiumAmount" name="premium_amount" value="${param.premium_amount}" required readonly /> 

        <label>Nhập số tiền nộp</label>
        <input type="text" id="paidAmount" name="paid_amount" value="${param.paid_amount}" required />

        <label>Trả theo</label>
        <select name="payment_frequency">
            <option value="monthly" ${param.payment_frequency == 'monthly' ? 'selected' : ''}>Tháng</option>
            <option value="quarterly" ${param.payment_frequency == 'quarterly' ? 'selected' : ''}>Quý</option>
            <option value="annually" ${param.payment_frequency == 'annually' ? 'selected' : ''}>Năm</option>
        </select>

        <label>Ngày bắt đầu hợp đồng vay</label>
        <input type="text" id="start_date" name="start_date" value="${param.start_date}" required readonly />

        <label>Ngày kết thúc hợp đồng vay</label>
        <input type="text" id="end_date" name="end_date" value="${param.end_date}" required readonly />
