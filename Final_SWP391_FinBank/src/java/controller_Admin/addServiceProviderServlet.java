 ...
 * @author DELL
 */
@WebServlet(name="addServiceProviderServlet", urlPatterns={"/addServiceProvider"})
public class addServiceProviderServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet addServiceProviderServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet addServiceProviderServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            ServiceProvider Phone = dao.getServiceProviderByPhone(email);
            ServiceProvider Name = dao.getServiceProviderByName(username);
            if (Username != null) {
                request.setAttribute("error", "Username " + username + " existed!!");
                request.getRequestDispatcher("addServiceProvider.jsp").forward(request, response);
            } else if (Email != null) {
                ...
                ServiceProvider sp = new ServiceProvider(7, username, 
                        username, hashedPassword, servicetype, email, 
                        phone_number, address, status);
                dao.insertServiceProvider(sp);
