...
public class UpdateLoanRequestServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int month;
        java.sql.Date currentDate = java.sql.Date.valueOf(LocalDate.now()); // date now
        ...
        if (status.equals("approved")) {
            try {
                ...
                d.InsertTransaction(Integer.parseInt(customerId), 2, l.getAmount(), "deposit");
                ...
                // Thêm dữ liệu vào bảng debt_management
                String insertDebtSql = "INSERT INTO debt_management (customer_id, loan_id, debt_status, overdue_days, notes) "
                        + "VALUES (?, ?, 'good', 0, ?)";
                try (Connection conn2 = db.getConnection();
                     PreparedStatement pstmt2 = conn2.prepareStatement(insertDebtSql)) {
                    pstmt2.setInt(1, customerid);  // customer_id
                    pstmt2.setInt(2, loan_id);     // loan_id
                    pstmt2.setString(3, "Khoản vay đã được duyệt"); // Ghi chú
                    pstmt2.executeUpdate(); // Thực hiện insert
                } catch (SQLException e) {
                    e.printStackTrace();
                }

                // Thông báo đến khách hàng về khoản vay đã duyệt
            ...
            sd.insertNotification(Integer.parseInt(customerId), Integer.parseInt(loan_id_raw), "Thông báo khác"
                    , "Đơn vay của bạn đã bị từ chối ! Nếu bạn muốn sử dụng lại dịch vụ, hãy điền và gửi lại form.");
        }
        response.sendRedirect("customerrequest");
    }
