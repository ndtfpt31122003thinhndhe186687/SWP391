...
public class ServiceTermDAO {
    ...
    public List<ServiceTerm> getLoanTerms() {
        List<ServiceTerm> list = new ArrayList<>();
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(SELECT_LOAN_TERMS);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ServiceTerm term = extractServiceTermFromResultSet(rs);
                list.add(term);
            }
    ...
    public List<ServiceTerm> getSavingTerms() {
        List<ServiceTerm> list = new ArrayList<>();
        try (Connection conn = getConnection();
    ...
    public ServiceTerm getTermById(int termId) {
        ServiceTerm term = null;
        try (Connection conn = getConnection();
    ...
    private ServiceTerm extractServiceTermFromResultSet(ResultSet rs) throws SQLException {
