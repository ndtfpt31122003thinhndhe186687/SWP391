    ...
    <div class="row">
        ...
        <main class="main-wrapper col-md-9 ms-sm-auto py-4 col-lg-9 px-md-4 border-start">
            <div class="title-group mb-3">
                <h1 class="h2 mb-0 text-danger">Quản lý bảo hiểm</h1>
            </div>

            <input type="hidden" name ="page" value="1">           
            <c:if test="${requestScope.insurance!=null}">    
                <div class="filter-sort-bar">
                    <label for="filterStatus">Lọc theo trạng thái:</label>
                    <select id="filterStatus" class="filter-dropdown" onchange="filterInsurance()">
                    ...
                    <label for="sortInsurance">Sắp xếp:</label>
                    <select id="sortInsurance" class="filter-dropdown">
                        <option value="insurance_name" ${requestScope.sort == 'insurance_name' ? 'selected' : ''}>Tên</option>
                        <option value="email" ${requestScope.sort == 'email' ? 'selected' : ''}>Email</option>                   
                    </select>

                    <label for="selectPageInsurance">Hiện:</label>
                    <select id="selectPageInsurance" class="filter-dropdown" onchange="selectPageInsurance()">
            ...
            <!-- View list service -->
             <c:if test="${requestScope.insurance!=null}">    
                <div class="mt-3">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <form action="searchInsurance" class="d-flex">                              
                        ...
                        <div class="col-md-2">
                            <a class="btn btn-success w-100" href="addInsurance.jsp">Thêm mới</a>
                        </div>
                    </div>
                            ...
                            <c:choose>
                                <c:when test="${not empty ListByName}">
                                    <c:forEach items="${ListByName}" var="s">
                                        <tr>
                                            <td>${s.insurance_id}</td>
                                            <td>${s.insurance_name}</td>
                                            <td>${s.email}</td>
                                            ...
                                            <td>
                                                <a onclick="doDeleteService('${s.insurance_id}')" href="#" class="btn btn-danger btn-sm">
                                                    <i class="bi bi-trash"></i>
                                                </a>
                                                <a href="updateInsurance?id=${s.insurance_id}" class="btn btn-success btn-sm">
                                ...
                                <c:otherwise>
                                    <c:forEach items="${insurance}" var="s">
                                        <tr>
                                            <td>${s.insurance_id}</td>
                                            <td>${s.insurance_name}</td>
                                            <td>${s.email}</td>
            ...
            <div class="pagination">
                <c:forEach begin="1" end="${totalPage}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="insurance_management?&status=${status}&sort=${sort}&page=${i}&pageSize=${pageSize}">${i}</a>     
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
            </div>
        ...
        <script type="text/javascript">
            ...
            function selectPageInsurance() {
                var status = document.getElementById("filterStatus").value;
                var sort = document.getElementById("sortInsurance").value;
                var pageSize = document.getElementById("selectPageInsurance").value;
                window.location.href = "insurance_management?status=" + status + "&sort=" + sort + "&page=1" + "&pageSize=" + pageSize;
            }
            ...
            document.getElementById("sortInsurance").onchange = sortInsurance;
