...
public class ServiceTermDAO {
    ...
    private static final String SELECT_LOAN_TERMS = "SELECT st.*, t.duration "
            + "FROM service_terms st "
            + "JOIN term t ON st.term_id = t.term_id "
            + "WHERE st.service_id = 2 AND st.status = 'active'";
    private static final String SELECT_SAVING_TERMS = "SELECT st.*, t.duration "
            + "FROM service_terms st "
            + "JOIN term t ON st.term_id = t.term_id "
            + "WHERE st.service_id = 1 AND st.status = 'active'";
    private static final String SELECT_TERM_BY_ID
            = "SELECT st.*, t.duration "
            + "FROM service_terms st "
            + "JOIN term t ON st.term_id = t.term_id "
            + "WHERE st.term_id = ?";
    ...
    public List<ServiceTerms> getLoanTerms() {
        ...
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_LOAN_TERMS); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    public List<ServiceTerms> getSavingTerms() {
        List<ServiceTerms> list = new ArrayList<>();
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_SAVING_TERMS); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ServiceTerms term = extractServiceTermFromResultSet(rs);
                list.add(term);
            }
    ...
    public ServiceTerms getTermById(int termId) {
        ...
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_TERM_BY_ID)) {
            ps.setInt(1, termId);
