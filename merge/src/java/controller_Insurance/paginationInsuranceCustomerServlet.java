 ...
 * @author Windows
 */
@WebServlet(name="paginationInsuranceCustomerServlet", urlPatterns={"/paginationInsuranceCustomer"})
public class paginationInsuranceCustomerServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet paginationInsuranceCustomerServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet paginationInsuranceCustomerServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        int count = dao.getTotalInsuranceCustomer(i.getInsurance_id());
        int endPage = count / quantity;
        if (count % quantity != 0) {
            endPage++;
        }
        list = dao.paginationInsuranceCustomer(i.getInsurance_id(), offset, quantity);
