...
<html>
    ...
    <body>
        <div class="container">
            <h1>Cập nhật thông tin bảo hiểm</h1>
            <h4 class="error-message">${requestScope.error}</h4>
            <c:set var ="i" value="${requestScope.insurance}"/>
            <form action="updateInsurance" method="post" id="insuranceForm" onsubmit="return validateForm()">
                <input type="hidden" name="id" value="${i.insurance_id}"/>
                <div class="form-group">
                    <label for="insurance_name">Họ và tên:</label>
                    <input type="text" id="insurance_name" name="insurance_name"  required
                           value ="${i.insurance_name}"/>
                    <div class="invalid-feedback" id="insurance_name-feedback">Hãy điền đúng tên hợp lệ.</div>
                </div>
        ...
        <script>
            function validateForm() {
                ...
                const full_name = document.getElementById('insurance_name');
                const full_nameFeedback = document.getElementById('insurance_name-feedback');
