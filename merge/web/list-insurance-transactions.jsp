...
<html lang="en">
    <head>
        ...
        <title>Mini Finance - List of Insurance Transactions</title>
        <!-- CSS FILES -->
            ...
            <div class="row">
                ...
                <nav id="sidebarMenu" class="col-md-3 col-lg-3 d-md-block sidebar collapse">
                    <div class="position-sticky py-4 px-3 sidebar-sticky">
                        <ul class="nav flex-column h-100">              
                            ...
                            <c:if test="${sessionScope.account.role_id==4}">
                                <li class="nav-item">
                                    <a class="nav-link active" href="list-insurance-transactions">
                                        <i class="bi-person me-2"></i>
                ...
                <main class="main-wrapper col-md-9 ms-sm-auto py-4 col-lg-9 px-md-4 border-start">
                    <div class="title-group mb-3">
                        <h1 class="h2 mb-0 text-danger">Danh sách các giao dịch bảo hiểm</h1>
                    </div>

                    <!-- Search Form -->
                    <div class="mb-4">
                        <form method="get" action="list-insurance-transactions" class="form-inline">
                            <div class="input-group">
                                <input type="text" name="search" class="form-control" placeholder="Tìm kiếm theo tên khách hàng hoặc loại giao dịch" value="${search}" />
                                ...
                                <tbody>
                                    <c:forEach var="transaction" items="${transactions}">
                                        <tr>
                                            ...
                                            <td>${transaction.contractInfo}</td>
                                            <td>
                                                <fmt:formatDate value="${transaction.transaction_date}" pattern="dd-MM-yyyy"/>
                                            </td>
                                            <td class="${transaction.amount >= 0 ? 'amount-positive' : 'amount-negative'}">
                                                <fmt:formatNumber value="${transaction.amount}" type="currency" currencySymbol="VNĐ"/>
                                            </td>
                                            <td>
                                                <c:choose>
                    ...
                    <nav aria-label="Page navigation" class="mt-4">
                        <ul class="pagination justify-content-center">
                            <c:if test="${currentPage > 1}">
                                <li class="page-item">
                                    <a class="page-link" href="list-insurance-transactions?page=${currentPage - 1}&search=${search}">Previous</a>
                                </li>
                            </c:if>
                            <c:forEach begin="1" end="${totalPages}" var="i">
                                <li class="page-item ${i == currentPage ? 'active' : ''}">
                                    <a class="page-link" href="list-insurance-transactions?page=${i}&search=${search}">${i}</a>
                                </li>
                            </c:forEach>
                            <c:if test="${currentPage < totalPages}">
                                <li class="page-item">
                                    <a class="page-link" href="list-insurance-transactions?page=${currentPage + 1}&search=${search}">Next</a>
                                </li>
                            </c:if>
                        </ul>
                    </nav>
